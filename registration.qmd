...
breaks, lunchs and social events. Register until the **18th of
...
|       Master's and doctoral students       |               190€               |              240€               |
:::
:::
...
<script type="module">
  ...
  document.getElementById("registrationForm").addEventListener("submit", async function(event) {
    ...
    if (!isPresenting) {
    // If not presenting a poster, remove required attributes
    conditionalFields.forEach(id => {
        document.getElementById(id).removeAttribute("required");
    });
    sessionRadios.forEach(radio => radio.removeAttribute("required"));
    } else {
    // Add required attributes if presenting
    conditionalFields.forEach(id => {
        document.getElementById(id).setAttribute("required", "required");
    });
    sessionRadios.forEach(radio => radio.setAttribute("required", "required"));
    }
